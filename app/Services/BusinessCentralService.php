...
class BusinessCentralService
{
    ...
    public function __construct()
    {
        ...
        // $this->url2 = 'http://192.168.1.16:7047/BC260/WS/TPG/Codeunit/APIMngt';
        $this->username = 'ahmed';
...
public function CustomerList_all()
{
    // Corps de la requête SOAP
    $request = <<<XML
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:b2b="urn:microsoft-dynamics-schemas/codeunit/B2BWS">
...
XML;
    ...
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $request);
    curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: text/xml; charset=utf-8', 'SOAPAction: urn:microsoft-dynamics-schemas/codeunit/B2BWS:CustomerList2']);
    ...
    $result = $body->children()->CustomerList2_Result->vARJson;
...
public function CreateCreditMemoForItemWithQty($postedSalesInvoiceNo, $itemNo, $creditQty)
{
    $soapRequest = <<<XML
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:b2b="urn:microsoft-dynamics-schemas/codeunit/B2BWS">
   <soapenv:Header/>
   <soapenv:Body>
      <b2b:CreateCreditMemoForItemWithQty>
         <b2b:postedSalesInvoiceNo>{$postedSalesInvoiceNo}</b2b:postedSalesInvoiceNo>
         <b2b:itemNo>{$itemNo}</b2b:itemNo>
         <b2b:creditQty>{$creditQty}</b2b:creditQty>
      </b2b:CreateCreditMemoForItemWithQty>
   </soapenv:Body>
</soapenv:Envelope>
XML;
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        'Content-Type: text/xml; charset=utf-8',
        'SOAPAction: urn:microsoft-dynamics-schemas/codeunit/B2BWS:CreateCreditMemoForItemWithQty'
    ]);
    ...
    try {
        ...
        $body = $xml->children($namespaces['Soap'])->Body;
        $result = $body->children('urn:microsoft-dynamics-schemas/codeunit/B2BWS')->CreateCreditMemoForItemWithQty_Result;

        // Dans ce cas, il n'y a pas de données renvoyées mais on vérifie simplement le succès de l'opération
        return ['success' => true, 'message' => 'Note de crédit créée avec succès.'];
    } catch (\Exception $e) {
        return ['success' => false, 'message' => 'Erreur traitement SOAP : ' . $e->getMessage()];
    }
}
...
public function CreateTotalCreditMemo($postedSalesInvoiceNo)
{
    $soapRequest = <<<XML
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:b2b="urn:microsoft-dynamics-schemas/codeunit/B2BWS">
   <soapenv:Header/>
   <soapenv:Body>
      <b2b:CreateCreditMemoFromPostedSalesInvoice>
         <b2b:postedSalesInvoiceNo>{$postedSalesInvoiceNo}</b2b:postedSalesInvoiceNo>
      </b2b:CreateCreditMemoFromPostedSalesInvoice>
   </soapenv:Body>
</soapenv:Envelope>
XML;
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $this->url); // ← Assure-toi que $this->url est bien défini dans le constructeur
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_USERPWD, $this->username . ':' . $this->password);
    curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_NTLM);
    curl_setopt($ch, CURLOPT_POST, true);
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        'Content-Type: text/xml; charset=utf-8',
        'SOAPAction: urn:microsoft-dynamics-schemas/codeunit/B2BWS:CreateCreditMemoFromPostedSalesInvoice'
    ]);
    ...
    try {
        ...
        $namespaces = $xml->getNamespaces(true);
        $body = $xml->children($namespaces['Soap'])->Body;
        $result = $body->children('urn:microsoft-dynamics-schemas/codeunit/B2BWS')->CreateCreditMemoFromPostedSalesInvoice_Result;

        return ['success' => true, 'message' => 'Avoir total généré avec succès.'];
    } catch (\Exception $e) {
        return ['success' => false, 'message' => 'Erreur traitement SOAP : ' . $e->getMessage()];
    }
}
