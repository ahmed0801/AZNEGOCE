    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="createItemModalLabel">Créer un Groupe TVA</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
            </div>

            <div class="modal-body">
                <form id="createItemForm" action="{{ route('grouptva.store') }}" method="POST">
                    @csrf
...
s
                        <div class="mb-3 col-md-8">
                            ...
                            <input type="text" class="form-control" id="description" name="code" required>
                        </div>
                ...
                @foreach ($grouptvas as $grouptva)
                    <tr>
                        ...
                        <td>
                            <!-- Modifier -->
<!-- Modifier -->
<button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editItemModal{{ $grouptva->id }}">
    <i class="fas fa-edit"></i>
</button>


                            <!-- Supprimer -->
                            <form action="{{ route('grouptva.destroy', $grouptva->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer cette Categorie ?')">
                                @csrf
                                @method('DELETE')
...
<div class="modal fade" id="editItemModal{{ $grouptva->id }}" tabindex="-1" aria-labelledby="editItemModalLabel{{ $grouptva->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">

            <div class="modal-header">
                <h5 class="modal-title">Modifier la Catégorie : {{ $grouptva->name }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
            </div>

            <div class="modal-body">
                <form action="{{ route('grouptva.update', $grouptva->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3 col-md-8">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ $grouptva->name }}" required>
                        </div>

                        <!-- Code -->
                        <div class="mb-3 col-md-4">
                            <label class="form-label">Pourcent.%</label>
                            <input type="number" name="rate" class="form-control" value="{{ $grouptva->rate }}" required>
                        </div>

                         <div class="mb-3 col-md-4">
                            <label class="form-label">Code</label>
                            <input type="text" name="code" class="form-control" value="{{ $grouptva->code }}" required>
                        </div>
