                        ...
                        <tr data-vendor="{{ $item['VendorName'] ?? '' }}">
                            ...
                            <td>
                                @php
                                    ...
                                    $margePctU = $unitCost > 0 ? ($marge / $unitCost) * 100 : 0;
                                @endphp
                                <input type="number" step="0.001" 
                                       ...
                                       📊 Marge U : {{ number_format($marge, 3, ',', ' ') }} ({{ number_format($margePctU, 1, ',', ' ') }}%)
                                       </div>
...
📊 Marge Total : {{ number_format($marge, 3, ',', ' ') }} ({{ number_format($margePctU, 1, ',', ' ') }}%)
</div>
                            </td>
...
let margePctU = unitCost > 0 ? (margeUnitaire / unitCost) * 100 : 0;
let margePctTotal = unitCost > 0 ? (margeTotale / (unitCost * quantite)) * 100 : 0;

    // MAJ marge unitaire et totale
    const margeText = `📊 Marge U : ${margeUnitaire.toFixed(3).replace('.', ',')} (${margePctU.toFixed(1).replace('.', ',')}%)`;
document.getElementById('marge-display-' + itemNo).innerText = margeText;

const margetotaltext = `📊 Marge Total : ${margeTotale.toFixed(3).replace('.', ',')} (${margePctTotal.toFixed(1).replace('.', ',')}%)`;
