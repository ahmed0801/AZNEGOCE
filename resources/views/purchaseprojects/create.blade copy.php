    ...
    <div class="page-inner">
        ...
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white">
                        <h5 class="mb-0">Créer un projet de commande</h5>
            </div>
            <div class="card-body">

               <form action="{{ route('purchaseprojects.store') }}" method="POST" enctype="multipart/form-data" id="projectForm">
                            @csrf
                            <div class="row mb-3">
                                <div class="col-md-4">
                                    ...
                                    <select name="supplier_id" class="form-control select2" required>
                                        @foreach($suppliers as $supplier)
                                            <option value="{{ $supplier->id }}" {{ session('imported_supplier_id') == $supplier->id ? 'selected' : '' }}>
                                                {{ $supplier->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                        ...
                                        @foreach(session('imported_lines') as $i => $line)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <input type="hidden" name="lines[{{ $i }}][sale_price]" class="sale-price" value="{{ $line['sale_price'] }}">
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endif
                            ...
                            <div class="text-end">
                                ...
                                <button type="submit" name="action" value="convert" class="btn btn-info px-4 ms-2 convert-to-order">🔄 Convertir en Commande</button>
                            </div>
                        </form>
        ...
        $(document).ready(function () {
            ...
            $('.convert-to-order').on('click', function () {
                swal({
                    ...
                    text: "Voulez-vous convertir ce projet en commande d'achat ?",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                }).then((willConvert) => {
                    if (willConvert) {
                        $('#projectForm').attr('action', '{{ route("purchaseprojects.convert") }}').submit();
                    }
                });
            });
