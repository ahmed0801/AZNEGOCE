...
<div class="u-text">
    ...
    <div class="page-inner">
        ...
        <h4>📋 Liste des projets de commande :
                ...
                <form method="GET" action="{{ route('purchaseprojects.list') }}" class="d-flex flex-wrap align-items-end gap-2 mb-3">
                    ...
                    <select name="status" class="form-select form-select-sm" style="width: 170px;">
                        <option value="">Statut projet (Tous)</option>
                    ...
                    <!-- <button type="submit" name="action" value="export" formaction="{{ route('purchaseprojects.export') }}"
                        class="btn btn-outline-success btn-sm px-3">
                        <i class="fas fa-file-excel me-1"></i> Exporter
                    </button> -->

                    <a href="{{ route('purchaseprojects.list') }}" class="btn btn-outline-secondary btn-sm px-3">
                ...
                @foreach ($projects as $project)
                    <div class="card mb-4 shadow-sm border-0">
                        <div class="card-header bg-white d-flex justify-content-between align-items-center border-start border-4 border-primary">
                            <div>
                                <h6 class="mb-0">
                                    <strong>Projet N° : {{ $project->numdoc }}</strong> –
                                    {{ $project->supplier->name }}
                                    <span class="text-muted small">({{ \Carbon\Carbon::parse($project->order_date)->format('d/m/Y') }})</span>
                                </h6>
                                @if($project->status === 'brouillon')
                                    <span class="badge bg-secondary">{{ ucfirst($project->status) }}</span>
                                @elseif($project->status === 'validée')
                            ...
                            <div class="btn-group">
                                <button class="btn btn-sm btn-outline-primary" onclick="toggleLines({{ $project->id }})">
                                    ➕ Détails
                                </button>
                                <a href="{{ route('purchaseprojects.export_single', $project->id) }}" class="btn btn-xs btn-outline-success">
                                ...
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu">
                                        @if($project->status !== 'converted')
                                            <form action="{{ route('purchaseprojects.convert', $project->id) }}" method="POST" onsubmit="return confirm('Convertir ce projet en commande d\'achat ?')" class="d-inline">
                                                @csrf
                                                <button type="submit" class="dropdown-item">
                                                    <i class="fas fa-arrow-right"></i> Convertir en commande
                                                </button>
                                            </form>
                                        @else
                                            <span class="dropdown-item disabled">
                                                <i class="fas fa-arrow-right"></i> Déjà converti
                                            </span>
                                        @endif
                                        @if($project->status === 'brouillon')
                                            <a class="dropdown-item" href="{{ route('purchaseprojects.edit', $project->id) }}">
                        ...
                        <div id="lines-{{ $project->id }}" class="card-body d-none bg-light">
                            <h6 class="fw-bold mb-3">🧾 Lignes du projet</h6>
                            ...
                            <div class="text-end mt-3">
                                <div class="p-3 bg-white border rounded d-inline-block">
                                    <strong>Total HT :</strong> {{ number_format($project->total_ht, 2) }} €<br>
                                    <strong>Total TTC :</strong> {{ number_format($project->total_ttc, 2) }} €
                                </div>
                            </div>
                        </div>
                    </div>
