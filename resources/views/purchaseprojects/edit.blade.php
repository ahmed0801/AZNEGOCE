            ...
            <div class="sidebar-wrapper scrollbar scrollbar-inner">
                <div class="sidebar-content">
                    <ul class="nav nav-secondary">
                        ...
                        <li class="nav-item active"><a href="/purchaseprojects/list"><i class="fas fa-file-alt"></i><p>Projets de Commande</p></a></li>
            ...
            <div class="page-inner">
                ...
                <div class="card shadow-sm">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0">Modifier le projet de commande : {{ $project->numdoc }}</h5>
                    </div>
                    <div class="card-body">
                        <!-- Shared Supplier Selection -->
                        <div class="row mb-3">
                            <div class="col-md-4">
                                <label class="form-label">Fournisseur</label>
                                <select id="shared_supplier_id" class="form-control select2" required>
                                    @foreach($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ $project->supplier_id == $supplier->id ? 'selected' : '' }}>
                                            {{ $supplier->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <!-- Import Form -->
                        <form action="{{ route('purchaseprojects.import') }}" method="POST" enctype="multipart/form-data" class="mb-3">
                            @csrf
                            <input type="hidden" name="supplier_id" id="import_supplier_id">
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label class="form-label">Importer depuis Excel</label>
                                    <input type="file" name="file" class="form-control" accept=".xlsx,.xls">
                                    <a href="{{ route('purchaseprojects.export-template') }}" class="btn btn-link">Télécharger le modèle Excel</a>
                                </div>
                                <div class="col-md-2">
                                    <button type="submit" class="btn btn-outline-primary mt-4">Importer</button>
                                </div>
                            </div>
                        </form>

                        <!-- Update Form -->
                        <form action="{{ route('purchaseprojects.update', $project->id) }}" method="POST" id="projectForm">
                            @csrf
                            <input type="hidden" name="supplier_id" id="save_supplier_id">
                            <div class="row mb-3">
                                <div class="col-md-2">
                                    <label class="form-label">TVA %</label>
                                    <input type="text" id="tva_display" class="form-control" readonly value="{{ $project->tva_rate }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Date de commande</label>
                                    <input type="date" name="order_date" class="form-control" value="{{ $project->order_date }}" required>
                                </div>
                            </div>
                            ...
                            <table class="table table-bordered" id="linesTable">
                                ...
                                <tbody id="lines">
                                    @if(session('imported_lines'))
                                        @foreach(session('imported_lines') as $i => $line)
                                            <tr>
                                                <td>
                                                    <select name="lines[{{ $i }}][article_code]" class="form-control select2-article" required>
                                                        <option value="{{ $line['article_code'] }}" selected>
                                                            {{ $line['article_code'] }} - {{ \App\Models\Item::where('code', $line['article_code'])->first()->name ?? '' }}
                                                        </option>
                                                    </select>
                                                </td>
                                                ...
                                                <td><input type="text" name="lines[{{ $i }}][tva]" class="form-control tva_ligne" readonly></td>
                            ...
                            <div class="row align-items-center mb-4">
                                <div class="col-md-6">
                                    ...
                                    <textarea name="notes" class="form-control" rows="3" placeholder="Remarques internes, conditions de livraison, etc.">{{ $project->notes }}</textarea>
                                </div>
                                <div class="col-md-6 text-end">
                                    <div class="p-3 bg-light border rounded shadow-sm">
                                        <h5 class="mb-1">Total HT : <span id="grandTotal" class="text-success fw-bold">{{ number_format($project->total_ht, 2) }}</span> €</h5>
                                        <h6 class="mb-0">Total TTC : <span id="grandTotalTTC" class="text-danger fw-bold">{{ number_format($project->total_ttc, 2) }}</span> €</h6>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="text-end">
                                ...
                                <button type="submit" name="action" value="validate" class="btn btn-success px-4 ms-2">✔️ Valider le Projet</button>
                                <button type="submit" name="action" value="convert" class="btn btn-info px-4 ms-2">🔄 Convertir en Commande</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
        let lineIndex = {{ count(session('imported_lines', [])) }};
        const tvaMap = {!! $tvaRates !!};

        $(document).ready(function () {
            $('.select2').select2({ width: '100%' });

            // Synchronize supplier ID across forms
            function syncSupplierId() {
                const supplierId = $('#shared_supplier_id').val();
                $('#import_supplier_id').val(supplierId);
                $('#save_supplier_id').val(supplierId);
            }

            // Initialize supplier selection
            syncSupplierId();
            $('#shared_supplier_id').on('change', function () {
                syncSupplierId();
                recalculate();
            });
            ...
            $('#shared_supplier_id').trigger('change');
            recalculate();
        });
    </script>
