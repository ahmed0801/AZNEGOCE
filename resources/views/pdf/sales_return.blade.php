...
<head>
    <meta charset="UTF-8">
    <title>Retour de Vente #{{ $return->numdoc }}</title>
    <style>
...
<body>
    <header>
        ...
        <h1>{{ $company->name }}</h1>
        <h4>Retour de Vente N° : {{ $return->numdoc }}</h4>
    ...
    <main>
        <table class="info-table">
            <tr>
                <td><strong>Client :</strong> {{ $return->customer->name ?? '-' }}</td>
                <td><strong>Date Retour :</strong> {{ \Carbon\Carbon::parse($return->return_date)->format('d/m/Y') }}</td>
            </tr>
            <tr>
                <td><strong>N° Client :</strong> {{ $return->customer->code ?? '-' }}</td>
                <td><strong>Type :</strong> {{ ucfirst($return->type) }}</td>
            </tr>
            <tr>
                <td><strong>Bon de Livraison :</strong> {{ $return->deliveryNote->numdoc ?? '-' }}</td>
                <td><strong>Statut :</strong> {{ $return->invoiced ? 'Facturé' : 'Non facturé' }}</td>
            </tr>
            <tr>
                <td colspan="2"><strong>Notes :</strong> {{ $return->notes ?? '-' }}</td>
            </tr>
        </table>
            ...
            <tbody>
                @foreach ($return->lines as $line)
                    <tr>
                        ...
                        <td>{{ number_format(($line->total_ligne_ht * (1+$return->tva_rate/100)), 2, ',', ' ') }} €</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="totals-box">
            <table>
                <tr>
                    ...
                    <td>{{ number_format($return->total_ht, 2, ',', ' ') }} €</td>
                </tr>
                <tr>
                    <td class="label">TVA ({{ number_format($return->tva_rate, 2, ',', ' ') }}%) :</td>
                    <td>{{ number_format($return->total_ttc - $return->total_ht, 2, ',', ' ') }} €</td>
                </tr>
                <tr>
                    <td class="label">Total TTC :</td>
                    <td>{{ number_format($return->total_ttc, 2, ',', ' ') }} €</td>
                </tr>
            </table>
        </div>
