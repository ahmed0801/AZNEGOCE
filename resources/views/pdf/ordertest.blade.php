...
<header>
    ...
    <h4>{{ $totalttc < 0 ? 'Bon de Retour' : 'Bon de Livraison' }} N° : {{ $NumBL }}</h4>
</header>
...
<main>
    ...
    <tr>
            <td><strong>Num. Document :</strong> {{ $NumBL }}</td>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach ($orderDetails as $item)
                ...
                <tr>
                    <td>{{ $item['NumArticle'] }}</td>
                    <td>{{ $item['Description'] }}</td>
                    <td>{{ number_format($item['PrixUnitaire'], 3, ',', ' ') }}</td>
                    <td>{{ $item['Quantite'] }}</td>
                    <td>{{ $item['Remise'] }}%</td>
                    <td>{{ number_format($item['totalHT'], 3, ',', ' ') }}</td>
                    <td>{{ $item['TVA'] }}%</td>
                    <td>{{ number_format($item['totalTTC'], 3, ',', ' ') }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="totals-box">
        <table>
            ...
            <tr>
                ...
                <td><strong>{{ number_format($totalttc, 3, ',', ' ') }} TND</strong></td>
            </tr>
        </table>
    </div>
</main>
