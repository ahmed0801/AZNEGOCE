...
<body>
    ...
    <main>
        <table class="info-table">
            ...
            <tr>
                <td><strong>TVA Fournisseur :</strong> {{ number_format($return->purchaseOrder->supplier->tvaGroup->rate ?? ($return->total_ht > 0 ? ($return->total_ttc / $return->total_ht - 1) * 100 : 0), 2, ',', ' ') }}%</td>
                <td><strong>Commande associée :</strong> 
                    @if($return->purchaseOrder)
                        {{ $return->purchaseOrder->numdoc }}
                    @else
                        Généré automatiquement à partir d'un Avoir Achat
                    @endif
                </td>
            </tr>
        </table>
