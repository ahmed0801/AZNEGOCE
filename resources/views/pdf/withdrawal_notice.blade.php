...
<head>
    <meta charset="UTF-8">
    <title>Avis de retrait #{{ $purchase->numdoc }}</title>
    <style>
...
<body>
    <header>
        ...
        <h1>{{ $company->name }}</h1>
        <h4>Avis de retrait N° : AR/{{ $purchase->numdoc }}</h4>
    ...
    <main>
        <table class="info-table">
            ...
            <tr>
                <td><strong>N° Fournisseur :</strong> {{ $purchase->supplier->code ?? '-' }}</td>
                <td><strong>Adresse :</strong> {{ $purchase->supplier->address ?? '-' }}</td>
            </tr>
        </table>
