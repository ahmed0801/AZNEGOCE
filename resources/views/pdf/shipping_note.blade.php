...
<head>
    <meta charset="UTF-8">
    <title>Bordereau d'envoi #{{ $deliveryNote->numdoc }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .comment-section { margin-top: 20px; padding: 10px; border: 1px solid #2c3e50; background-color: #f8f9fa; }
        .comment-section p { margin: 0; }
...
<body>
    <header>
        ...
        <h1>{{ $company->name }}</h1>
        <h4>Bordereau d'envoi N° : BE/{{ $deliveryNote->numdoc }}</h4>
    ...
    <main>
        <table class="info-table">
            <tr>
                ...
                <td><strong>Date BL :</strong> {{ \Carbon\Carbon::parse($deliveryNote->delivery_date)->format('d/m/Y') }}</td>
            </tr>
            <tr>
                <td><strong>N° Client :</strong> {{ $deliveryNote->numclient ?? '-' }}</td>
                <td><strong>Adresse :</strong> {{ $deliveryNote->address ?? '-' }}</td>
            </tr>
        </table>
