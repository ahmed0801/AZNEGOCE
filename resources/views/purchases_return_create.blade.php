            ...
            <div class="sidebar-wrapper scrollbar scrollbar-inner">
                <div class="sidebar-content">
                    <ul class="nav nav-secondary">
                        ...
                        <li class="nav-item active"><a href="/returns"><i class="fas fa-undo-alt"></i><p>Retours Achat</p></a></li>
...
<div class="u-text">
    ...
    <form action="{{ route('logout.admin') }}" method="POST" style="display: inline;">
        ...
        <div class="page-inner">
            ...
            <h4>📦 Créer un retour pour la commande #{{ $order->numdoc }}</h4>
            <p>Fournisseur : {{ $order->supplier->name }} | Date : {{ \Carbon\Carbon::parse($order->order_date)->format('d/m/Y') }}</p>

            <form action="{{ route('purchases.return.store', $order->id) }}" method="POST">
                @csrf
                <input type="hidden" name="supplier_id" value="{{ $order->supplier->id }}">

                <div class="mb-3">
                    <label for="return_date" class="form-label">Date du retour</label>
                    <input type="date" name="return_date" id="return_date" class="form-control" value="{{ now()->format('Y-m-d') }}" required>
                </div>

                <div class="mb-3">
                    <label for="type" class="form-label">Type de retour</label>
                    <select name="type" id="type" class="form-select select2" onchange="toggleLinesSection()">
                        <option value="total">Retour total</option>
                        <option value="partiel">Retour partiel</option>
                    </select>
                </div>

                <div id="lines-section" class="mb-3 d-none">
                    <h5>Lignes à retourner</h5>
                    @foreach ($order->lines as $index => $orderLine)
                        <div class="line-row mb-2">
                            <input type="checkbox" name="lines[{{ $orderLine->article_code }}][selected]" value="1" onchange="toggleQuantityInput(this, '{{ $orderLine->article_code }}')">
                            <label>{{ $orderLine->item->name ?? $orderLine->article_code }} (Qté commandée: {{ $orderLine->ordered_quantity }})</label>
                            <input type="number" name="lines[{{ $orderLine->article_code }}][returned_quantity]" 
                                   class="form-control quantity-input" step="0.01" min="0" 
                                   max="{{ $orderLine->ordered_quantity }}" disabled
                                   placeholder="Quantité (max {{ $orderLine->ordered_quantity }})">
                            <input type="hidden" name="lines[{{ $orderLine->article_code }}][article_code]" value="{{ $orderLine->article_code }}">
                            @error("lines.{$orderLine->article_code}.returned_quantity")
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save"></i> Créer le retour
                </button>
                <a href="{{ route('purchases.list') }}" class="btn btn-secondary">
...
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
    ...
    <script>
        ...
        function toggleQuantityInput(checkbox, articleCode) {
            const quantityInput = document.querySelector(`input[name="lines[${articleCode}][returned_quantity]"]`);
            quantityInput.disabled = !checkbox.checked;
            if (!checkbox.checked) {
                quantityInput.value = '';
            } else {
                const maxQty = parseFloat(quantityInput.getAttribute('max')) || 0;
                quantityInput.addEventListener('input', function () {
                    const enteredQty = parseFloat(this.value) || 0;
                    if (enteredQty > maxQty) {
                        this.value = maxQty;
                        alert('La quantité à retourner ne peut pas dépasser la quantité commandée.');
                    }
                });
            }
        }
