...
<!-- End Sidebar -->
    ...
    <div class="main-panel">
        <div class="main-header">
            <!-- Navbar (identique à ta page Articles) -->
            <nav class="navbar navbar-header navbar-header-transparent navbar-expand-lg border-bottom">
                <div class="container-fluid">
                    <ul class="navbar-nav topbar-nav ms-md-auto align-items-center">
                        <!-- [TON NAVBAR COMPLET] -->
                        <!-- Quick Actions, Profil, etc. -->
                        <!-- Copie-colle depuis ta page Articles -->
                    </ul>
                </div>
            </nav>
        </div>
    ...
    Aller a la page Articles ⇒
                        ...
                        <a href="{{ route('articles.import.template') }}" class="btn btn-primary mb-3">
                            <i class="fas fa-download me-2"></i> Télécharger le modèle Excel (Exemplaire)
                        </a>

                        <form id="importForm" action="{{ route('articles.import') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
                                <label for="file" class="form-label">Choisir le fichier Excel :</label>
...
<!-- TON SCRIPT ORIGINAL (100% FONCTIONNEL) -->
<script>
document.getElementById('fileInput').addEventListener('change', function(e) {
    ...
    .catch(err => {
        ...
        preview.innerHTML += '<div class="alert alert-danger">Erreur de prévisualisation. Vérifiez le format du fichier.</div>';
    });
});
</script>
