...
@if (!empty($avoirs))
    ...
    <tbody>
        @foreach ($avoirs as $avoir)
            <tr>
                <td>{{ $avoir['NumFacture'] }}</td>
                <td>{{ $avoir['DateFacture'] }}</td>
                <td>{{ $avoir['CodeClient'] }} &nbsp;&nbsp;-&nbsp;&nbsp; {{ $avoir['NomClient'] }}</td>
                <td>{{ is_numeric($avoir['MontantHT']) ? number_format($avoir['MontantHT'], 3) . ' TND' : $avoir['MontantHT'] . ' TND' }}</td>
                <td>{{ is_numeric($avoir['MontantTTC']) ? number_format($avoir['MontantTTC'], 3) . ' TND' : $avoir['MontantTTC'] . ' TND' }}</td>
                <td>
                                @if (strtolower($avoir['Payed']) == "true")
                            ...
                            <td>
                    <a href="{{ route('avoirs.show', ['NumAvoir' => $avoir['NumFacture']]) }}" 
                       class="btn btn-info btn-sm" 
                       title="Voir les détails de l'avoir">
                        <i class="fas fa-eye"></i> Voir
                    </a>
                </td>
                <td>
                    <a href="{{ route('avoirs.export_pdf', ['NumAvoir' => $avoir['NumFacture'],'CustomerNo' => $avoir['CodeClient'],'DateFacture' => str_replace('/', '-', $avoir['DateFacture'])]) }}" 
                       class="btn btn-success btn-sm" 
                       title="Exporter cet avoir en PDF" target="_blank">
    ...
    @else
        <p class="text-center text-muted">Aucun Avoir disponible.</p>
    @endif
</div>
