...
<!-- fin recherche dans le tableau -->
    ...
    <tr>
        ...
        <th style="display: none;">Prix V.HT</th>
        <th>Prix V.TTC</th>
        <th>Rem%</th>
...
<td>
    ...
    @php
        ...
        $priceTTC = round($priceHT * 1.19, 3);
        $unitCost = floatval($item['Unit Cost'] ?? 0);
        $marge = $priceHT - $unitCost;
        $margePourcent = $unitCost > 0 ? ($marge / $unitCost) * 100 : 0;
    @endphp
    ...
    <input type="number" step="0.001" 
        ...
        value="{{ $priceTTC }}"
        style="width: 100px; margin-left: 5px; font-size: 14px;"
        oninput="updateHT('{{ $item['ItemNo'] }}')">

    <!-- Hidden Unit Cost -->
    <input type="hidden" id="unitCost_{{ $item['ItemNo'] }}" value="{{ $unitCost }}">

    <!-- Input HT (optionnel si tu en as un) -->
    <input type="hidden" id="prixv-{{ $item['ItemNo'] }}" value="{{ $priceHT }}">

    <!-- Marge display -->
    <div id="marge-display-{{ $item['ItemNo'] }}" class="text-muted" style="font-size: 11px; margin-top: 2px;">
        📊 Marge: {{ number_format($marge, 3, ',', ' ') }} HT 
        ({{ number_format($margePourcent, 1, ',', ' ') }} %)
    </div>
</td>
...
function updateHT(itemNo) {
    const ttcInput = document.getElementById(`prixvttc-${itemNo}`);
    const htInput = document.getElementById(`prixv-${itemNo}`);
    const unitCost = parseFloat(document.getElementById(`unitCost_${itemNo}`)?.value || 0);

    const prixTTC = parseFloat(ttcInput.value);
    if (!isNaN(prixTTC)) {
        const prixHT = +(prixTTC / 1.19).toFixed(3);
        htInput.value = prixHT;

        // Recalcul marge
        const marge = +(prixHT - unitCost).toFixed(3);
        const margePourcent = unitCost > 0 ? ((marge / unitCost) * 100).toFixed(1) : 0;

        // Mettre à jour l'affichage
        const margeDisplay = document.getElementById(`marge-display-${itemNo}`);
        if (margeDisplay) {
            margeDisplay.innerHTML = `📊 Marge : ${marge.toLocaleString('fr-FR', {minimumFractionDigits: 3})} DT (${margePourcent} %)`;
        }
    }
}
</script>
