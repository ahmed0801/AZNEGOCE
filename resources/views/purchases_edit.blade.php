...
<h5 class="mb-0">✏️ Modifier la commande #{{ $order->numdoc }}</h5>
          </div>
          <div class="card-body">

<form action="{{ route('purchases.update', $order->numdoc) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="row mb-3">
                                <div class="col-md-6">
                            ...
                            <table class="table table-bordered" id="linesTable">
                                ...
                                <tbody id="lines">
                                    @foreach($order->lines as $i => $line)
                                        <tr>
                                            <td>
                                                <select name="lines[{{ $i }}][article_code]" class="form-control select2-article" required>
                                                    <option value="{{ $line->article_code }}" selected>
                                                        {{ $line->article_code }} - {{ $line->article->name ?? '' }}
                                                    </option>
                                                </select>
                                            </td>
                                            <td><input type="number" name="lines[{{ $i }}][ordered_quantity]" class="form-control qty" value="{{ $line->ordered_quantity }}" required></td>
                                            <td><input type="number" step="0.01" name="lines[{{ $i }}][unit_price_ht]" class="form-control pu" value="{{ $line->unit_price_ht }}" required></td>
                                            <td><input type="number" step="0.01" name="lines[{{ $i }}][remise]" class="form-control remise" value="{{ $line->remise }}"></td>
                                            <td><input type="number" step="0.01" name="lines[{{ $i }}][tva]" class="form-control tva" value="{{ $line->tva ?? ($order->supplier->tvaGroup->rate ?? 0) }}" readonly></td>
                                            <td><input type="text" class="form-control total_ht" readonly></td>
                                            <td><input type="text" class="form-control total_ttc" readonly></td>
                                            <td>
                                                <button type="button" class="btn btn-outline-danger btn-sm remove-line">×</button>
                                                <button type="button" class="btn btn-outline-primary btn-sm edit-sale-price" data-index="{{ $i }}"><i class="fas fa-euro-sign"></i></button>
                                                <input type="hidden" name="lines[{{ $i }}][sale_price]" class="sale-price" value="{{ $line->article->sale_price ?? 0 }}">
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            ...
                            <div class="row align-items-center mb-4">
                                ...
                                <div class="col-md-6 text-end">
                                    <div class="p-3 bg-light border rounded shadow-sm">
                                        <h5 class="mb-0">Total HT : <span id="grandTotal" class="text-success fw-bold">0.00</span> €<br>
                                            Total TTC : <span id="grandTotalTTC" class="text-primary fw-bold">0.00</span> €
                                        </h5>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="text-end">
                                @if($order->status === 'brouillon')
                                    <button type="submit" class="btn btn-info px-4">💾 Mettre à jour</button>
                                    <button type="submit" name="action" value="validate" class="btn btn-success px-4">✅ Mettre à jour & Valider</button>
                                @endif
                                <a href="{{ route('purchases.list') }}" class="btn btn-secondary">Retour</a>
                            </div>
                        </form>
...
<script>
        let lineIndex = {{ count($order->lines) }};
        ...
        function initSelect2Article(selector, defaultSalePrice = 0) {
            $(selector).select2({
                ajax: {
                ...
                const purchasePrice = parseFloat(data.price || 0);
                const salePrice = parseFloat(data.sale_price || defaultSalePrice);
                $row.find('.pu').val(purchasePrice.toFixed(2));
                $row.find('.sale-price').val(salePrice.toFixed(2));
                $row.find('.edit-sale-price').prop('disabled', false);
                recalculate();
            });
        }
        ...
        function recalculate() {
            ...
            $('#lines tr').each(function () {
                ...
                $(this).find('.tva').val(tva.toFixed(2));
                $(this).find('.total_ht').val(lineHT.toFixed(2));
                $(this).find('.total_ttc').val(lineTTC.toFixed(2));
                totalHT += lineHT;
            });
        ...
        $(document).ready(function () {
            $('.select2').not('.select2-article').select2({ width: '100%' });

            $('.select2-article').each(function (index) {
                const salePrice = $(this).closest('tr').find('.sale-price').val();
                initSelect2Article(this, salePrice);
            });
            ...
            $('#addLine').click(function () {
                ...
                const newRow = `
                    <tr>
                        ...
                        <td><input type="number" step="0.01" name="lines[${lineIndex}][tva]" class="form-control tva" value="${tva.toFixed(2)}" readonly></td>
                        <td><input type="text" class="form-control total_ht" readonly></td>
                        <td><input type="text" class="form-control total_ttc" readonly></td>
                        <td>
                            <button type="button" class="btn btn-outline-danger btn-sm remove-line">×</button>
                            <button type="button" class="btn btn-outline-primary btn-sm edit-sale-price" data-index="${lineIndex}" disabled><i class="fas fa-euro-sign"></i></button>
                            <input type="hidden" name="lines[${lineIndex}][sale_price]" class="sale-price">
                        </td>
                    </tr>`;
                $('#lines').append(newRow);
                initSelect2Article(`select[name="lines[${lineIndex}][article_code]"]`);
                lineIndex++;
                recalculate();
            });
            ...
            $('#saveSalePrice').click(function () {
                ...
                const $row = $(`input[name="lines[${index}][sale_price]"]`).closest('tr');
                $(`input[name="lines[${index}][sale_price]"]`).val(newSalePrice.toFixed(2));
                $row.find('.edit-sale-price').removeClass('btn-outline-primary').addClass('btn-primary').html('<i class="fas fa-check"></i>');
