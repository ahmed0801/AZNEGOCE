                    ...
                    @foreach($items as $item)
                        <tr data-vendor="{{ $item['VendorName'] ?? '' }}">
                            ...
                            <td>
                                ...
                                <br>
                                <small class="text-muted" style="font-size: 9px;">
                                    📍FOUR: {{ $item['VendorName'] ?? 'Empl. vide' }}
                                </small>
                            </td>
...
$(document).ready(function () {
    ...
    function applyFilters() {
        ...
        tbody.find('tr').each(function () {
            const row = $(this);
            const stockCell = row.find('td:nth-child(4) svg'); // Stock est maintenant la 4e colonne
            const vendorName = row.attr('data-vendor')?.trim() || '';
            const isInStock = stockCell.length && stockCell.hasClass('bi-patch-check-fill');
            const textMatch = row.text().toLowerCase().indexOf(searchValue) > -1;
            ...
            row.toggle(textMatch && stockMatch && vendorMatch);
        });
    }
...
function updateTTC(itemNo) {
    ...
    // Mettre à jour l'affichage du prix TTC
    document.getElementById('prixvttc-' + itemNo).value = prixTTCApresRemise.toFixed(3);
    ...
    document.getElementById('prix-ttc-apres-remise-' + itemNo).innerText = prixTTCApresRemise.toFixed(3).replace('.', ',') + ' TTC';
}
