    ...
    <form action="{{ route('logout.admin') }}" method="POST" style="display: inline;">
        ...
        <div class="container">
          <div class="page-inner">
            ...
            <h4>Détails de la Commande : {{ $orderNo}}</h4>
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach ($orderDetails as $order)
                                <tr>
                                    <td>{{ $order['NumArticle'] }}</td>
                                    <td>{{ $order['Description'] }}</td>
                                    <td>{{ $order['Quantite'] }}</td>
                                    <td>{{ is_numeric($order['PrixUnitaire']) ? number_format($order['PrixUnitaire'], 3) . ' TND' : $order['PrixUnitaire'] . ' TND' }}</td>
                                    <td>{{ is_numeric($order['totalHT']) ? number_format($order['totalHT'], 3) . ' TND' : $order['totalHT'] . ' TND' }}</td>
                                    <td>{{ is_numeric($order['totalTTC']) ? number_format($order['totalTTC'], 3) . ' TND' : $order['totalTTC'] . ' TND' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
                <a href="/orders" class="btn btn-primary mt-4">
                    <i class="fas fa-arrow-left"></i> Retour vers mes commandes
                </a>
              @else
