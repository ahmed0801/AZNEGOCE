...
$(document).ready(function () {
            ...
            processResults: function (data) {
                ...
                return {
                    results: data.map(item => ({
                        id: item.id,
                        text: `${item.type === 'order' ? 'Commande' : 'Retour'} #${item.numdoc} - ${item.supplier_name} (${new Date(item.order_date).toLocaleDateString()})`,
                        type: item.type,
                        lines: item.lines,
    ...
    function getTVA() {
        ...
        // Validate that all items have the same tva_rate
                ...
                const row = `
                    <tr>
                        <td><input type="text" value="${item.text}" class="form-control" readonly>
                            <input type="hidden" name="lines[${lineIndex}][${item.type === 'order' ? 'purchase_order_id' : 'purchase_return_id'}]" value="${item.id.replace(/^(order|return)_/, '')}">
                        </td>
