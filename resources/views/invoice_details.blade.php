                        ...
                        <tbody>
                            @foreach ($invoiceDetails as $invoice)
                                <tr>
                                    ...
                                    <td>
    <form action="{{ route('create.credit.memo') }}" method="POST" onsubmit="return confirm('⚠️ Confirmez-vous la création de l’avoir ?');">
        @csrf
        <input type="hidden" name="NumFacture" value="{{ $NumFacture }}">
        <input type="hidden" name="CodeArticle" value="{{ $invoice['CodeArticle'] }}">

        <div class="input-group input-group-sm">
            <input type="number" name="Quantite" class="form-control form-control-sm" min="1" max="{{ $invoice['Quantite'] }}" value="1" required style="max-width: 60px;">
            <button type="submit" class="btn btn-danger btn-sm text-white" title="Faire Avoir">
