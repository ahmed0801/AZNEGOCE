...
<!-- fin test panier -->
    ...
    <form action="{{ route('logout.admin') }}" method="POST" style="display: inline;">
        ...
        <h4>Détails de l'Avoir : {{ $NumAvoir }}</h4>
    ...
    <table class="table table-bordered table-hover">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ is_numeric($detail['PrixUnitaire']) ? number_format($detail['PrixUnitaire'], 3) . ' TND' : $detail['PrixUnitaire'] . ' TND' }}</td>
                <td>{{ is_numeric($detail['MontantHT']) ? number_format($detail['MontantHT'], 3) . ' TND' : $detail['MontantHT'] . ' TND' }}</td>
                <td>{{ is_numeric($detail['MontantTTC']) ? number_format($detail['MontantTTC'], 3) . ' TND' : $detail['MontantTTC'] . ' TND' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
</div>
<a href="{{ route('avoirs.index') }}" class="btn btn-primary mt-4">
    <i class="fas fa-arrow-left"></i> Retour vers mes Avoirs
</a>
@else
<p class="text-center text-muted">Aucun détail disponible pour cet avoir.</p>
@endif
</div>
