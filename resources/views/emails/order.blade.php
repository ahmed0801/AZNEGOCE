...
<head>
    <meta charset="UTF-8">
    <title>Document {{ $order->numdoc }}</title>
    <style>
...
<body>
    <div class="container">
        <h2>Bonjour {{ $order->customer->name ?? 'Client' }},</h2>

        {{-- Message accompagnant (safety: escape + conserver sauts de ligne) --}}
        <p>{!! nl2br(e($messageText)) !!}</p>

        <p>
            Référence facture : <strong>{{ $order->numdoc }}</strong><br>
            Date : <strong>{{ optional( $order->created_at)->format('d/m/Y') ?? '-' }}</strong>
        </p>
