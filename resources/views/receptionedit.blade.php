...
<div class="u-text">
  ...
  <div class="page-inner">
      ...
      <div class="container">
      <h3>✏️ Modifier réception #{{ $reception->id }} (Commande : {{ $reception->purchaseOrder->numdoc }})</h3>
...
<div class="mb-3">
  ...
  <div id="scanner" style="display: none;">
      <div id="reader"></div>
      <button type="button" class="btn btn-danger mt-2" id="closeScanner">❌ Fermer Scanner</button>
  </div>
</div>
<style>
#scanner {
...
function handleScan(code) {
    ...
    if (!found) alert("Article non trouvé : " + code);
}
...
document.getElementById('openScanner').addEventListener('click', function () {
    ...
    // ⏱️ Disparaît après 1.5 secondes
    setTimeout(() => {
        notif.style.opacity = '0';
        setTimeout(() => document.body.removeChild(notif), 500);
    }, 1500);

    // 📷 Fermer automatiquement le scanner
...
document.getElementById('closeScanner').addEventListener('click', function () {
    if (html5QrCodeInstance) {
        html5QrCodeInstance.stop().then(() => {
            ...
            console.error("Erreur stop caméra : ", err);
        });
    }
});
</script>
