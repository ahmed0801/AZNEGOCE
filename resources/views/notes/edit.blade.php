    ...
    <div class="page-inner">
        ...
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0">Modifier l’avoir #{{ $note->numdoc }}</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('notes.update', $note->numdoc) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="type" value="{{ $note->type }}">
                    <input type="hidden" name="tva_rate" id="tva_rate" value="{{ $note->tva_rate }}">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            ...
                            <select name="supplier_id" class="form-control select2" required>
                                ...
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}" {{ $note->supplier_id == $supplier->id ? 'selected' : '' }}>
                                        {{ $supplier->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-2">
                            ...
                            <input type="text" id="tva_display" class="form-control" readonly value="{{ $note->tva_rate }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Date de l’avoir</label>
                            <input type="date" name="note_date" class="form-control" value="{{ $note->note_date }}" required>
                        </div>
                    </div>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        @if($line->purchase_return_id)
                                            <input type="hidden" name="lines[{{ $loop->index }}][purchase_return_id]" value="{{ $line->purchase_return_id }}">
                                        @endif
                                        @if($line->purchase_invoice_id)
                                            <input type="hidden" name="lines[{{ $loop->index }}][purchase_invoice_id]" value="{{ $line->purchase_invoice_id }}">
                                        @endif
                                    </td>
                                    <td><input type="number" name="lines[{{ $loop->index }}][quantity]" class="form-control qty" value="{{ $line->quantity }}" required></td>
                                    <td><input type="number" step="0.01" name="lines[{{ $loop->index }}][unit_price_ht]" class="form-control pu" value="{{ $line->unit_price_ht }}" required></td>
                                    <td><input type="number" step="0.01" name="lines[{{ $loop->index }}][remise]" class="form-control remise" value="{{ $line->remise }}"></td>
                                    <td><input type="text" name="lines[{{ $loop->index }}][tva]" class="form-control tva_ligne" value="{{ $line->tva }}" readonly></td>
                    ...
                    <div class="row align-items-center mb-4">
                        <div class="col-md-6">
                            ...
                            <textarea name="notes" class="form-control" rows="3" placeholder="Remarques internes, conditions de l’avoir, etc.">{{ $note->notes }}</textarea>
                        </div>
                        <div class="col-md-6 text-end">
                            <div class="p-3 bg-light border rounded shadow-sm">
                                <h5 class="mb-1">Total HT : <span id="grandTotal" class="text-success fw-bold">{{ number_format($note->total_ht, 2) }}</span> €</h5>
                                <h6 class="mb-0">Total TTC : <span id="grandTotalTTC" class="text-danger fw-bold">{{ number_format($note->total_ttc, 2) }}</span> €</h6>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function () {
    ...
    function recalculate() {
        let totalHT = 0;
        const tva = parseFloat($('#tva_rate').val()) || 0;
