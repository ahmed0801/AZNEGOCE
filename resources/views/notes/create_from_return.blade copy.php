    ...
    $('.select2-returns').select2({
        ajax: {
            ...
            processResults: function (data) {
                return {
                    results: data.filter(item => item.type === 'return').map(item => ({
                        id: item.id,
                        text: `Retour #${item.numdoc} - ${item.supplier_name} (${new Date(item.order_date).toLocaleDateString()})`,
                        type: item.type,
                        lines: item.lines,
    ...
    $('.select2-returns').on('change', function () {
        ...
        selectedItems.forEach(item => {
            item.lines.forEach(line => {
                const lineHT = line.ordered_quantity * line.unit_price_ht * (1 - (line.remise || 0) / 100);
                const lineTTC = lineHT * (1 + tva / 100);
                const row = `
                    <tr>
                        <td><input type="text" value="${item.text}" class="form-control" readonly>
                            <input type="hidden" name="lines[${lineIndex}][purchase_return_id]" value="${item.id.replace('return_', '')}">
                        </td>
                        ...
                        <td><input type="number" name="lines[${lineIndex}][quantity]" class="form-control qty" value="${line.ordered_quantity}" required></td>
