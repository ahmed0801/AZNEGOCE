            ...
            <div class="container">
                <div class="page-inner">
                    ...
                    <div class="container mt-4">
                        <h4>Rapprochement pour {{ $account->name }} ({{ $account->account_number }})</h4>
                        <p>Solde actuel: {{ number_format($account->balance, 2, ',', ' ') }} €</p>

                        <form action="{{ route('generalaccounts.storeReconciliation', $account->id) }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="balance" class="form-label">Ajuster le solde (optionnel, en €)</label>
                                <input type="number" step="0.01" name="balance" id="balance" class="form-control" value="{{ $account->balance }}" placeholder="Entrez un nouveau solde si nécessaire">
                                @error('balance')
                            ...
                            <div class="table-responsive">
                                <h5>Paiements non rapprochés</h5>
                                @if($payments->count() > 0)
                                    <table class="table table-bordered table-hover table-text-small">
                                        <thead class="table-dark">
                                            <tr>
                                                <th><input type="checkbox" id="select-all-payments" onclick="document.querySelectorAll('input[name=\'payment_ids[]\']').forEach(cb => cb.checked = this.checked);"></th>
                                                <th>Date</th>
                                                <th>Type</th>
                                                <th>Montant (€)</th>
                                                <th>Code Lettrage</th>
                                ...
                                @else
                                    <p>Aucun paiement non rapproché trouvé.</p>
                                @endif

                                <h5>Transferts non rapprochés</h5>
                                @if($transfers->count() > 0)
                                    <table class="table table-bordered table-hover table-text-small">
                                        <thead class="table-dark">
                                            <tr>
                                                <th><input type="checkbox" id="select-all-transfers" onclick="document.querySelectorAll('input[name=\'transfer_ids[]\']').forEach(cb => cb.checked = this.checked);"></th>
                                                <th>Date</th>
                                                <th>De</th>
                                                <th>Vers</th>
                                                <th>Montant (€)</th>
                                        ...
                                        <tbody>
                                            @foreach($transfers as $transfer)
                                                <tr>
                                                    ...
                                                    <td>{{ $transfer->fromAccount ? $transfer->fromAccount->name : '-' }}</td>
                                                    <td>{{ $transfer->toAccount ? $transfer->toAccount->name : '-' }}</td>
                                ...
                                @else
                                    <p>Aucun transfert non rapproché trouvé.</p>
                                @endif
                            </div>
                            <div class="mt-3">
                                <button type="submit" class="btn btn-success">Marquer comme rapproché</button>
