...
<button type="button" class="btn btn-primary mb-2" data-bs-toggle="modal" data-bs-target="#createSoucheModal">
    Créer une Souche
</button>

<!-- Modal Créer une Souche -->
<div class="modal fade" id="createSoucheModal" tabindex="-1" aria-labelledby="createSoucheModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="{{ route('souche.store') }}" method="POST">
                @csrf

                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">Créer une souche</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="mb-3 col-md-4">
                            ...
                            <input type="text" name="code" class="form-control" required placeholder="Ex: CLIENT">
                        </div>
                        <div class="mb-3 col-md-8">
                            <label class="form-label">Nom *</label>
                            <input type="text" name="name" class="form-control" required placeholder="Ex: Souche Client">
                        </div>
                        <div class="mb-3 col-md-4">
                            <label class="form-label">Préfixe *</label>
                            <input type="text" name="prefix" class="form-control" placeholder="Ex: CLT-" required>
                        </div>
                                                <div class="mb-3 col-md-4">
                            <label class="form-label">Suffixe</label>
                            <input type="text" name="suffix" class="form-control" placeholder="Ex : 25">
                        </div>
                        <div class="mb-3 col-md-4">
                            <label class="form-label">Longueur Numéro *</label>
                            <input type="number" name="number_length" class="form-control" value="5" required>
                        </div>
                        <div class="mb-3 col-md-6">
                            <label class="form-label">Dernier Numéro utilisé</label>
                            <input type="number" name="last_number" class="form-control" value="0">
                        </div>
                        <div class="mb-3 col-md-6">
                            ...
                            <select name="type" class="form-select" required>
                                ...
                                <option value="commande_vente">commande Vente</option>
                                <option value="bon_livraison">Bon Livraison</option>
                                <option value="retour_vente">Retour Vente</option>
                                <option value="commande_achat">Commande Achat</option>
                                <option value="retour_achat">Retour Achat</option>
                                <option value="avoir_vente">Avoir Vente</option>
                                <option value="avoir_achat">Avoir Achat</option>
                                <option value="projet_commande">Projet de Commande</option>
                            </select>
                        </div>
                    </div>
                </div>
...
<div class="mb-1 d-flex justify-content-center">
    <input type="text" id="searchItemInput" class="form-control search-box" placeholder="🔍 Rechercher une Souche...">
</div>
            ...
            <tbody>
                @foreach ($souches as $souche)
                    <tr>
                        ...
                        <td>{{ $souche->prefix }}</td>
                        <td>{{ $souche->suffix }}</td>
                        <td>{{ $souche->number_length }}</td>
                        <td>{{ $souche->last_number }}</td>
                        <td>{{ $souche->type }}</td>
                        <td>
                            <!-- Modifier -->
                            <button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editSoucheModal{{ $souche->id }}">
                                <i class="fas fa-edit"></i>
                            </button>

                            <!-- Modal Modifier -->
                            <div class="modal fade" id="editSoucheModal{{ $souche->id }}" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <form action="{{ route('souche.update', $souche->id) }}" method="POST">
                                            @csrf @method('PUT')

                                            <div class="modal-header">
                                                <h5 class="modal-title">Modifier la souche</h5>
                                            ...
                                            <div class="modal-body">
                                                <div class="row">
                                                    <div class="mb-3 col-md-4">
                                                        <label class="form-label">Code</label>
                                                        <input type="text" name="code" class="form-control" value="{{ $souche->code }}" required>
                                                    </div>
                                                    <div class="mb-3 col-md-8">
                                                        <label class="form-label">Nom</label>
                                                        <input type="text" name="name" class="form-control" value="{{ $souche->name }}" required>
                                                    </div>
                                                    <div class="mb-3 col-md-4">
                                                        <label class="form-label">Préfixe</label>
                                                        <input type="text" name="prefix" class="form-control" value="{{ $souche->prefix }}" required>
                                                    </div>
                                                    <div class="mb-3 col-md-4">
                                                        <label class="form-label">Suffixe</label>
                                                        <input type="text" name="suffix" class="form-control" value="{{ $souche->suffix }}">
                                                    </div>
                                                    <div class="mb-3 col-md-4">
                                                        <label class="form-label">Longueur Numéro</label>
                                                        <input type="number" name="number_length" class="form-control" value="{{ $souche->number_length }}">
                                                    </div>
                                                    
                                                    <div class="mb-3 col-md-6">
                                                        <label class="form-label">Dernier Numéro utilisé</label>
                                                        <input type="number" name="last_number" class="form-control" value="{{ $souche->last_number }}">
                                                    </div>
                                                    <div class="mb-3 col-md-6">
                                                        <label class="form-label">Type</label>
                                                        <input type="text" name="type" class="form-control" value="{{ $souche->type }}" disabled>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <!-- Supprimer -->
                            <form action="{{ route('souche.destroy', $souche->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer cette souche ?')">
                                @csrf @method('DELETE')
...
@else
    <p class="text-muted text-center">Aucune souche trouvée.</p>
@endif
